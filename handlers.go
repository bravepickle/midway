...
func NewLogger() *CurlLogger {
	...
	if !Config.Log.Request.Disabled {
		if Config.Log.Request.Output == `` {
			...
			f, err := os.OpenFile(Config.Log.Request.Output, os.O_APPEND|os.O_CREATE|os.O_EXCL, 0664)
			if err != nil {
				log.Fatalln(err)
			}
	...
	if !Config.Log.Response.Disabled {
		if Config.Log.Response.Output == `` {
			...
			f, err := os.OpenFile(Config.Log.Response.Output, os.O_APPEND|os.O_CREATE|os.O_EXCL, 0664)
			if err != nil {
				log.Fatalln(err)
			}
	...
	if Config.Log.ErrorLog == `` {
		...
		f, err := os.OpenFile(Config.Log.ErrorLog, os.O_APPEND|os.O_CREATE|os.O_EXCL, 0664)
		if err != nil {
			log.Fatalln(err)
		}
